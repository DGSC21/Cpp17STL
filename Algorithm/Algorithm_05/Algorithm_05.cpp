//정렬되거나 정렬되지 않은 벡터에서 요소 찾기(find, equal_range, ...)
#include<iostream>
#include<vector>
#include<list>
#include<algorithm>
...
int main()
{
	const vector<city> c
	{
		{"Aachen", 246000}, {"Berlin", 3502000}, {"Braunschweig", 251000}, {"Cologne", 1060000}
	};
	...
	{
		auto found_cologne(find_if(begin(c), end(c), [](const auto& aaa) {return aaa.name == "Cologne"; }));
		print_city(found_cologne);
	}
